...
class FlickrPanel
{
	...
	function func_PhotoInfo($pid, $defaultGroup)
	{
		global $flickr_manager;
		
		if(!empty($pid)) {
			
			if($flickr_manager->settings['streamlined_enable'] == true) {
				$page = new Page('templates/InsertPhotoStreamlined.tpl');
			} else {
			...
			$sizeMarkup = FlickrPanel::GetSizeOptions('imageSize', $sizes, $flickr_manager->settings['imagesize_default']);
	...
	function GetSizeOptions($name, $sizes, $default) {
		...
		foreach($sizes as $key=>$size) {
			$option = '<span class="image-size-item">
...
if(strtoupper($_SERVER['REQUEST_METHOD']) == "POST" && basename($_SERVER['REQUEST_URI']) == basename(__FILE__))
{
	...
	switch($_POST['func']) {
		case 'func.getPhotos':
			...
			break;
		case 'func.photoInfo':
			FlickrPanel::func_PhotoInfo($_POST['pid'], $_POST['defaultgroup']);
			break;
		case 'func.saveInfo':
